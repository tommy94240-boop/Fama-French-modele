...
#Pour lancer l'application, tapez dans le terminal : /usr/local/bin/python3 -m streamlit run "/Users/tommylouhichi/Python/Fama-French/FF5.py"
#Attention à bien copier le chemin d'accès selon l'utilisateur sur cette partie "/Users/tommylouhichi/Python/Fama-French/FF5.py"
...
st.title("Régression linéaire Fama-French 5 facteurs")
...
if chemin_fichier and rendements_portefeuille and date_debut and date_fin:
    ...
    tableau_final = pd.concat([nouveau_tableau_rendements, nouveau_tableau_FF5], axis=1)
    tableau_final['Excès de rendement'] = tableau_final['Rendement du portefeuille'] - tableau_final['RF']

    #Régression linéaire
    x = pd.concat([tableau_final['Mkt-RF'], tableau_final['SMB'], tableau_final['HML'], tableau_final['RMW'], tableau_final['CMA']], axis=1, ignore_index=True)
    x.columns = ("Prime de marché", "SMB", "HML", "RMW", "CMA")
    x = sm.add_constant(x)
    ...
    with st.expander("Alpha"):
        st.write("""
    L'Alpha représente la performance du gérant qui n'est pas expliquée par les 5 facteurs de risque :
    ...
    Le concept : Le test compare votre modèle (avec les 5 facteurs) à un modèle vide (qui dirait juste que le rendement est égal à l'Alpha moyen). 
